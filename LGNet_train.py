...
from options.options_EORSSD import opt
...
logging.basicConfig(
    filename=os.path.join(save_path, 'LGNet.log'),
...
logging.info("LGNet-Train_4_pairs")

model = LGNet(checkpoint_path=opt.load)
...
def train(train_loader, model, optimizer, epoch, save_path):
    ...
    try:
            ...
            if i % 100 == 0 or i == total_step or i == 1:
                ...
                logging.info(
                    ...
                    format(epoch, opt.epoch, i, total_step,
                           optimizer.state_dict()['param_groups'][0]['lr'], loss.data, memory_used))
        ...
        if (epoch) % 5 == 0:
            torch.save(model.state_dict(), save_path + 'LGNet_epoch_{}.pth'.format(epoch))
    except KeyboardInterrupt:
        print('Keyboard Interrupt: save model and exit.')
        torch.save(model.state_dict(), save_path + 'LGNet_epoch_{}.pth'.format(epoch + 1))
    ...
    with torch.no_grad():
        ...
        if epoch == 1:
            ...
            if mae < best_mae:
                best_mae = mae
                best_epoch = epoch
                torch.save(model.state_dict(), save_path + 'LGNet_epoch_best.pth')
